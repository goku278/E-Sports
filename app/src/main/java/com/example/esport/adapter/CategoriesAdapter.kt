...
import com.example.esport.model.Categories
...
class CategoriesAdapter(
    ...
    private val turfList: List<Turfs>,
    private val clickListener: TurfsListener
) : RecyclerView.Adapter<CategoriesAdapter.ResponseViewHolder>() {
        ...
        init {
            ...
            itemView.setOnClickListener {
                ...
                if (position != RecyclerView.NO_POSITION) {
                    val category = turfList[position]
                    clickListener.onCategoriesClicked(category)
                }
            }
        }

        // Method to bind the data from a Response object to the views
        fun bind(response: Turfs, position: Int) {
...
//            Glide.with(context).load(Constant.imageServer + response.icon).into(imageViewIcon)
    ...
    override fun onBindViewHolder(holder: ResponseViewHolder, position: Int) {
        holder.bind(turfList[position], position)
    }
